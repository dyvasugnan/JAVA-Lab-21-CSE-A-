...
public class StudentRegistrationForm extends Frame implements ActionListener, ItemListener{
	...
	Frame resultFrame; // New Frame to display the selected options
	Label resultLabel; // Label to show the selected options in the new Frame
	...
	StudentRegistrationForm(){
		...
		prog = new List(4,true);//true for multi selection and 4 rows
	...
	void initializeResultFrame() {
        resultFrame = new Frame("Selected Options");
        TextArea resultTextArea = new TextArea();
        resultTextArea.setEditable(false); // Make the TextArea non-editable so now its only display
	...
	//function on the new frame
	void showSelectedOptionsInFrame() {
     ...
     // provides dynamic string manipulation without performance overhead of String class
        ...
        resultTextArea.setEditable(false); // Make the TextArea non-editable
        resultFrame.add(resultTextArea, BorderLayout.CENTER); 
        resultFrame.setVisible(true); // Make the frame visible
    }
